...
public class AiExecutionService : IAiExecutionService, IDependency
{
    ...
    public async Task ExecuteAiTaskAsync(ExecutionCard card)
    {
        ...
        try
        {
            ...
            var existingPhases = await _taskPhaseProgressRepository.GetByExecutionCardIdAsync(card.Id);
            if (!existingPhases.Any())
            {
                ...
                foreach (DevelopmentPhase phase in Enum.GetValues(typeof(DevelopmentPhase)))
                {
                    ...
                    await _taskPhaseProgressRepository.InsertAsync(phaseProgress);
                }
            }

            // Find the first not started phase and mark it as InProgress
            var allPhases = await _taskPhaseProgressRepository.GetByExecutionCardIdAsync(card.Id);
            ...
            card = await _executionCardRepository.GetByIdAsync(card.Id);
        ...
        catch (Exception ex)
        {
            ...
            var cardRef = await _executionCardRepository.GetByIdAsync(card.Id);
